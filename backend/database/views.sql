...
SELECT
  ...
  COALESCE(SUM(CASE WHEN sp.status = 'confirmed' THEN sp.amount ELSE 0 END), 0) AS total_amount,
...
SELECT
  e.event_id,
  e.event_name,
  COALESCE(SUM(CASE WHEN s.status = 'confirmed' THEN s.amount ELSE 0 END), 0) AS approved_sponsorship_amount,
